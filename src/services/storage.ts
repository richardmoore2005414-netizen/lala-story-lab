...
// Storage keys bumped to v6 for full book preset to ensure clean adoption settings and direct install
const STORAGE_KEYS = {
  CHARACTERS: 'lala_story_characters_v6',
  MESSAGES: 'lala_story_messages_v6',
  BRANCH_NODES: 'lala_story_branch_nodes_v6',
  DIARIES: 'lala_story_diaries_v6',
  CORPUS: 'lala_story_corpus_v6',
  SESSION: 'lala_story_session_v6',
  PRESET_SCENES: 'lala_story_preset_scenes_v6',
  SCRIPTS: 'lala_story_scripts_v6',
...
// Only ban truly dummy / non-book placeholders
      ...
      for (const c of list) {
        if (c.id === 'adam') {
          if (
            ...
            c.title = '養子';
            c.relationshipWithLala = '領養關係（法定養子）。履行對已故師傅養到18歲的承諾。非親生，勿浪漫化母愛。';
            c.background = '法定養子。生父是Lala師傅（已故）。收養是履行「養到18歲」承諾的法律手段，非突發母愛。背景嚴格取自原著檔案。';
